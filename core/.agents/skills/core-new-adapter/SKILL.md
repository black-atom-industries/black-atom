...
12. **Create README.md** following the established pattern:
    - Header with Black Atom banner image
    - Description of what a Black Atom adapter is
    - Available themes table (all collections)
    - Installation prerequisites
    - Setup instructions (clone, generate)
    - Usage instructions (platform-specific methods for applying themes)
    - FAQ section (common issues like terminal background, tmux true color)
    - Development section (template structure, regenerating themes)
...
14. **Verify core recognizes the adapter**:

```bash
cd ~/repos/black-atom-industries/core && deno task adapters:status
```

15. **Summary**: Present to user:
    - Files created (including README)
    - Token mappings used

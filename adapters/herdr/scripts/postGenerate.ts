...
function readAdapterConfig(value: unknown): AdapterConfig {
    ...
    for (const [name, collectionValue] of Object.entries(value.collections)) {
        ...
        if (
            !collectionValue.themes.every((theme): theme is string => typeof theme === "string")
        ) {
...
const missingThemes = [...themeCollections.keys()].filter((theme) => !generatedThemeSet.has(theme));
if (missingThemes.length > 0) {
    console.warn(
        `Generated themes missing from this core version: ${missingThemes.join(", ")}`,
    );
}
